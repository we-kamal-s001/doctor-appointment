...
let model_namespace = 'VaahCms\\Modules\\appoinments\\Models\\DoctorPatientAppoinment';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/appoinments/appoinments";
...
export const useDoctorPatientAppoinmentStore = defineStore({
    id: 'appoinments',
    state: () => ({
        base_url: base_url,
        ajax_url: ajax_url,
    ...
    actions: {
        ...
        async getItemAfter(data, res) {
            if (data) {
                ...
                this.$router.push({name: 'appoinments.index', query: this.query});
            }
        ...
        async itemAction(type, item = null) {
            ...
            if (this.item?.doctor) {
                this.item.doctor_id = this.item.doctor?.id;
            }
        ...
        async formActionAfter(data) {
            switch (this.form.action) {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'appoinments.form', query: this.query, params: {id: null}});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm() {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'appoinments.form', query: this.query})
        },
        //---------------------------------------------------------------------
        toView(item) {
            ...
            this.$router.push({name: 'appoinments.view', params: {id: item.id}, query: this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item) {
            if (!this.item || !this.item.id || this.item.id !== item.id) {
            ...
            this.$router.push({name: 'appoinments.form', params: {id: item.id}, query: this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge() {
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useDoctorPatientAppoinmentStore, import.meta.hot))
}
